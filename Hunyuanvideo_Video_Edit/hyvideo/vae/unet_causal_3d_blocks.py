...
def replicate_pad_5d(x, padding):
    # padding: a 6-tuple for 5D tensor, in order (w_left, w_right, h_left, h_right, d_left, d_right)
    # x shape: (N, C, D, H, W)
    w_left, w_right, h_left, h_right, d_left, d_right = padding

    # Pad along W dimension
    if w_left > 0:
        left_pad = x[..., :1].repeat(1, 1, 1, 1, w_left)
        x = torch.cat([left_pad, x], dim=-1)
    if w_right > 0:
        right_pad = x[..., -1:].repeat(1, 1, 1, 1, w_right)
        x = torch.cat([x, right_pad], dim=-1)

    # Pad along H dimension
    if h_left > 0:
        top_pad = x[..., :1, :].repeat(1, 1, 1, h_left, 1)
        x = torch.cat([top_pad, x], dim=-2)
    if h_right > 0:
        bottom_pad = x[..., -1:, :].repeat(1, 1, 1, h_right, 1)
        x = torch.cat([x, bottom_pad], dim=-2)

    # Pad along D dimension
    if d_left > 0:
        front_pad = x[..., :1, :, :].repeat(1, 1, d_left, 1, 1)
        x = torch.cat([front_pad, x], dim=-3)
    if d_right > 0:
        back_pad = x[..., -1:, :, :].repeat(1, 1, d_right, 1, 1)
        x = torch.cat([x, back_pad], dim=-3)
...
class CausalConv3d(nn.Module):
    ...
    def forward(self, x):
        x = replicate_pad_5d(x, self.time_causal_padding)
        return self.conv(x)
